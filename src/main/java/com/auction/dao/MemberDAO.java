...
//	        pstmt.setString(2, userPwd);
	...
	public int enrollMember(Connection conn, MemberDTO m) {
		...
		String sql = "INSERT INTO USERS (MEMBER_ID, MEMBER_PWD, MEMBER_NAME, EMAIL, TEL, "
				+ "BIRTHDATE, GENDER, MOBILE_CARRIER, ENROLL_DATE, MILEAGE, " + "ZIP, ADDR1, ADDR2, MEMBER_TYPE) " // ←
																													// 여기
																													// 4개
																													// 추가!
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, SYSDATE, 0, ?, ?, ?, ?)";
	...
	public int updateMember(Connection conn, MemberDTO m) {
		...
		// 주소 정보도 함께 수정
		String sql = "UPDATE USERS SET " + "MEMBER_NAME = ?, " + "EMAIL = ?, " + "TEL = ?, " + "ZIP = ?, "
				+ "ADDR1 = ?, " + "ADDR2 = ? " + "WHERE MEMBER_ID = ?";
	...
	public int updatePassword(Connection conn, String memberId, String currentPwd, String newPwd) {
		...
		// 현재 비밀번호 확인 후 변경
		String sql = "UPDATE USERS SET MEMBER_PWD = ? " + "WHERE MEMBER_ID = ? AND MEMBER_PWD = ?";
